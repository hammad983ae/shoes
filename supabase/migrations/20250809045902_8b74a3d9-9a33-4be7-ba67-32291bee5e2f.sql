-- Create messages table with spam-related ip column
...
-- Update profiles with role and is_creator
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Functions to safely reference existing values without recursion
...
-- Tighten existing update policy: prevent non-admins changing privileged fields
...
-- Seed the admin user (email: stxrfo@gmail.com)
DO $$
DECLARE
  v_user_id uuid;
BEGIN
...
-- RPC: promote/demote creator guarded by admin check
...
-- RPC: set user role to 'user' or 'creator' (never 'admin')
